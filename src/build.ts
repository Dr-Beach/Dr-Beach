    ...
    .map(
      (role) => `
        <article class="role-card">
          <img src="${role.imageUrl}" alt="${escapeHtml(role.alt)}" />
...
<html lang="en">
  ...
  content.cta.label
      ...
      <footer class="footer">
        <span>${escapeHtml(content.siteTitle)}</span>
        <span>Hamburg, Germany - Algarve, Portugal</span>
      </footer>
    </div>
  </body>
</html>`;
...
.page {
  ...
  background: linear-gradient(180deg, rgba(0, 0, 0, 0.35), rgba(0, 0, 0, 0.8)),
    url("${content.backgroundUrl}") center/cover no-repeat fixed;
  display: flex;
